...
export const maleSwardonianWarriorNames = {
  type: "picker",
  list: [  "Grzmichuj",
  "Moczywór",
  "Dzierżytopór",
...
export const maleLatinNames = function () {
  ...
  const praenomen = randomizeFromArray(praenomina);
  const nomen = randomizeFromArray(nomina);

  // 70% pełne tria nomina, 30% krótsze imię dwuczłonowe.
  const includeCognomen = Math.random() < 0.7;

  const generatedName = includeCognomen
    ? `${praenomen} ${nomen} ${randomizeFromArray(cognomina)}`
    : `${praenomen} ${nomen}`;
